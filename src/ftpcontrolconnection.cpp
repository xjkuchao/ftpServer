...
FtpControlConnection::FtpControlConnection(QObject *parent, QSslSocket *socket,
...
bool FtpControlConnection::verifyAuthentication(const QString &command) {
    ...
    for (size_t ii = 0;
         ii < sizeof(commandsRequiringAuth) / sizeof(commandsRequiringAuth[0]);
         ++ii) {
...
bool FtpControlConnection::verifyWritePermission(const QString &command) {
    ...
    for (size_t ii = 0; ii < sizeof(commandsRequiringWritePermission) /
                                 sizeof(commandsRequiringWritePermission[0]);
         ++ii) {
...
void FtpControlConnection::parseCommand(const QString &entireCommand,
                                        QString *      command,
...
QString FtpControlConnection::toLocalPath(const QString &fileName) const {
    ...
    foreach (const QString &component,
             localPath.split('/', Qt::SkipEmptyParts)) {
        if (component == "..") {
...
void FtpControlConnection::port(const QString &addressAndPort) {
    ...
    qsizetype               ret = addressAndPort.indexOf(re, 0, &match);
    if (ret != -1) {
        QString hostName = match.captured(1).replace(',', '.');
        int port = match.captured(2).toInt() * 256 + match.captured(3).toInt();
        dataConnection->scheduleConnectToHost(hostName, port,
...
void FtpControlConnection::pasv() {
    ...
    reply(QString("227 Entering Passive Mode (%1,%2,%3).")
              .arg(socket->localAddress().toString().replace('.', ','))
              .arg(port / 256)
              .arg(port % 256));
}
...
void FtpControlConnection::rnto(const QString &fileName) {
    ...
    if ("RNFR" == command &&
        QDir().rename(toLocalPath(commandParameters), fileName)) {
        reply("250 Requested file action okay, completed.");
    } else {
        reply("550 Requested action not taken; file unavailable.");
    }
...
void FtpControlConnection::quit() {
    ...
    if (dataConnection->ftpCommand()) {
        connect(dataConnection->ftpCommand(), SIGNAL(destroyed()), this,
...
void FtpControlConnection::pass(const QString &password) {
    ...
    if (this->password.isEmpty() ||
        ("USER" == command && this->userName == commandParameters &&
         this->password == password)) {
