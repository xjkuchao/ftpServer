...
FtpRetrCommand::FtpRetrCommand(QObject *parent, const QString &fileName,
                               qint64 seekTo)
    : FtpCommand(parent) {
...
void FtpRetrCommand::startImplementation() {
    ...
    if (socket->isEncrypted()) {
        connect(socket, SIGNAL(encryptedBytesWritten(qint64)), this,
                SLOT(refillSocketBuffer(qint64)));
    } else {
