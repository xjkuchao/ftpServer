...
class FtpControlConnection : public QObject {
    Q_OBJECT
  public:
    explicit FtpControlConnection(QObject *parent, QSslSocket *socket,
  ...
  private:
    ...
    // parameters.
    void parseCommand(const QString &entireCommand, QString *command,
    ...
    // Flag whether the FTP client has logged (provided a username/password) or
    // not yet.
    bool isLoggedIn;
